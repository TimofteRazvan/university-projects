...
Controller::Controller(Repo repository, Cart* cart)
...
void Controller::addCoat(std::string size, std::string colour, int price, int quantity, std::string photograph)
{
	...
	this->undoAction.push_back(action);
}

void Controller::deleteCoat(std::string photo)
{
	RepositoryValidator::validate_deleteCoat(repo, photo);
	Coat deletedCoat = this->repo.getCoat(photo);
	repo.deleteCoat(photo);
	std::shared_ptr<UndoRedoAction> action = std::make_shared<UndoRedoRemove>(deletedCoat, this->repo);
	this->undoAction.push_back(action);
}
...
void Controller::updateCoat(std::string photo, std::string _size, std::string _colour, int _price, int _quantity, std::string _photo)
{
	...
	Coat oldCoat = this->repo.getCoat(photo);
	repo.updateCoat(photo, _size, _colour, _price, _quantity, _photo);
	std::shared_ptr<UndoRedoAction> action = std::make_shared<UndoRedoUpdate>(oldCoat, newCoat, this->repo);
	this->undoAction.push_back(action);
}
...
void Controller::addFilter(const Coat& newCoat)
{
	this->repo.addFilter(newCoat);
}
...
void Controller::writeCartToFile()
{
	cart->writeToFile();
}

void Controller::readCartFromFile()
{
	cart->readFromFile();
}

void Controller::addToAbsCart(Coat& coat)
{
	cart->addToCart(coat);
}

void Controller::filterClear()
{
	this->repo.filterClear();
}
...
void Controller::undo()
{
	...
	this->undoAction.back()->undo();
	this->redoAction.push_back(this->undoAction.back());
	this->undoAction.pop_back();
}
...
void Controller::redo()
{
	...
	this->redoAction.back()->redo();
	this->undoAction.push_back(this->redoAction.back());
	this->redoAction.pop_back();
}

/*
void Controller::clearUndoRedo()
{
	this->undoAction.clear();
	this->redoAction.clear();
}
*/

/*
void Controller::sortRepo(std::vector<Coat> elements, Comparator<Coat, Coat>* comptype)
{
	sortAll(elements, comptype);
}
*/
